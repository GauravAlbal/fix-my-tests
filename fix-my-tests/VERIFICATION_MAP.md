...
Use after a session when the user needs an implementation-ready evidence plan. One row per material bad outcome; never one row per existing test file.
...
2. Hard invariants cannot be offset by other passing tests.
...
7. A characterization baseline can support an explicit preservation claim; it does not become binding correctness merely because it existed before.
...
Select only rows that control a hard invariant or materially change the user's next decision.
